...
gfbiPlotsTable = fread("BiomassCalculationData/GFBI_Plots_with_All_Years_Time_Series_Records_20190412_Lidong.csv")[,-1]
# plot a figure for the distribution of plotArea
pdf(paste("Plots/HistGram_of_Plot_Size_Full_Data_All_Years.pdf",sep=""))
hist((gfbiPlotsTable$plotArea),main="Hist of plot size (ha)",xlab="Plot Size",ylab="Frequency",breaks=100)
abline(v = log(100), col="red", lwd=1, lty=2)
abline(v = log(10000), col="red", lwd=1, lty=2)
dev.off()
...
write.csv(currentYearPlotsTable,"BiomassCalculationData/GFBI_Plots_Biomass_Data_with_Full_Data_Year_Recording_20200520_PlotSize_Cleaned.csv")
...
cleanedPlotsTable = fread("BiomassCalculationData/GFBI_Plots_Biomass_Data_with_Full_Data_Year_Recording_20200520_PlotSize_Cleaned.csv")[,-1]

WWF_Biome = raster("WWFRaster/WWF_Raster_Uniformed_Extent_Uniformed.tif")
...
write.csv(cleanedPlotsTable,"BiomassCalculationData/GFBI_Plots_Biomass_Data_with_Full_Data_Year_Recording_20200520_Biome_Defined.csv")
...
# load the table 
currentYearPlotsTable = fread("BiomassCalculationData/GFBI_Plots_Biomass_Data_with_Full_Data_Year_Recording_20200520_Biome_Defined.csv")[,-1] %>% filter(!is.na(WWF_Biome)) 

# define the forest type vector
biomeVector = c(1,2,4,5,6,7,8,10,11,12,13)
biomeNames = c("Tropical moist","Tropical dry","Temperate broadleaf","Temperate coniferous","Boreal","Tropical savanna","Temperate savanna","Montane grassland","Tundra","Mediterranean forest","Desert")
pdf(paste("Plots/Figure_S3_HistGram_of_Tree_density_for_Biome_Level_LOG_Transed_MAD_25_PlotSize_Filtered_20200525_FullData_AllYear.pdf",sep=""))
...
for (bm in biomeVector)
{
    ...
    # transfer the unit from lg/ha biomass to t/ha Carbon
    ...
    # # make a histgram
    hist(biomeSubsetTable$TreePerAreea,main=paste(biomeNames[which(biomeVector==bm)],sep=""),xlab="Tree density(log(N)/ha)",ylab="Frequency")
    ...
    print(paste("Biome: ",biomeNames[which(biomeVector==bm)],sep=""))
    print((nrow(madFilteredTable)/nrow(biomeSubsetTable)))
    write.csv(madFilteredTable,paste("BiomassCalculationData/TreeDensityFilter_SubTables/Full_Data_all_Year_25_PlotSize_Filtered_Biome_Level_Table_for_",bm,"_Biome_NON_Log.csv",sep=""))
}

mtext("Log tranformed (filtered 3 MAD of treePerArea)", line=0, side=3, outer=TRUE, cex=1.5)
dev.off()
...
fileNameVector = list.files(path="BiomassCalculationData/TreeDensityFilter_SubTables",pattern = "_Biome_NON_Log.csv",full.name=T)
...
# write to local folder
write.csv(rbindedTable,"BiomassCalculationData/GFBI_Plots_Biomass_Density_Full_Data_TreeDensity_filtered_Recording_20200520_MAD_25_AllYear.csv")  
...
library(rgdal)
# read the data
filteredTable = fread("BiomassCalculationData/GFBI_Plots_Biomass_Density_Full_Data_TreeDensity_filtered_Recording_20200520_MAD_25_AllYear.csv")[,-1]
...
writeRaster(recordsRaster,paste("BiomassCalculationData/Rasters/Full_Data_Retained_Plots_with_TreeDensity_25_MAD_Filter_Mean_after_AllYear.tif",sep=""),overwrite=T)
...
recordsRaster = raster("BiomassCalculationData/Rasters/Full_Data_Retained_Plots_with_TreeDensity_25_MAD_Filter_Mean_after_AllYear.tif")
...
write.csv(rasterizedDataFram,"BiomassCalculationData/Full_Data_Aggregated_Retained_Plots_with_TreeDensity_MAD_25_Filtered_Biomass_Density_Mean_20200526_AllYear.csv")
...
rasterizedDataFram = fread ("BiomassCalculationData/Full_Data_Aggregated_Retained_Plots_with_TreeDensity_MAD_25_Filtered_Biomass_Density_Mean_20200526_AllYear.csv")[,-1]
# duplicate the table 
...
writeOGR(rasterizedDataFram,dsn="Full_Shapefile",layer="GFBI_Full_MAD25_AllYear_Mean_Aggre", driver = "ESRI Shapefile",overwrite=T)
