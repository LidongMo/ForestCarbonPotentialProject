...
setwd("/Users/LeonidMoore/Desktop/BIOMASS/Github_Folder")
...
dbh_path                     <- paste("input_data/20180201_Randomized_Tropical_DBH_Matrix_1.csv")
dbh_matrix                   <- read.csv(dbh_path)
#Load the dbh data based on individual level#
dbh_sp_path                  <- paste("input_data/20180208_GFBI_Randomized_DBH_Tree_List.csv")
dbh_sp_matrix                <- read.csv(dbh_sp_path)

#Load the wood density data matrix#
wd_path                      <- paste("input_data/GlobalWoodDensityDatabase_data.csv")
...
write.csv(dbh_df,"input_data/species_name_dbh.csv")

wd_df                        <- data.frame(binomial=sp_name_wd)
write.csv(wd_df,"input_data/species_name_wd.csv")

individual_df                <- data.frame(binomial=sp_individual_dbh)
write.csv(individual_df,"input_data/species_name_dbh_individual.csv")
...
tnrs_dbh_path                <- paste("input_data/tnrs_results_dbh.csv")
tnrs_sp_name_dbh             <- read.csv(tnrs_dbh_path)[-1,]

tnrs_wd_path                 <- paste("input_data/tnrs_results_wd.csv")
tnrs_sp_name_wd              <- read.csv(tnrs_wd_path)[-1,]

tnrs_dbh_indi_path           <- paste("input_data/tnrs_results_dbh_individual.csv")
tnrs_sp_name_dbh_indi        <- read.csv(tnrs_dbh_indi_path)[-1,]

tnrs_gfbi_path               <- paste("input_data/tnrs_results_GFBI_list.csv")
...
write.csv(GFBIRawAndCorrectedName,"input_data/GFBI_raw_and_tnrs_corrected_binomial_df.csv")
...
write.csv(correc_wd_matrix,"Resources/name_corrected_wooddensity_database.csv",row.names = FALSE)

#modiciaction the wood density data sources of the original getWoodDensity function in BIOMASS packages#
source("Resources/getWoodDensity_MO.r")
...
write.csv(corrected_wd_gfbi,"output_data/original_wd_of_all_spsecies_GFBI.csv")
table(corrected_wd_gfbi$levelWD)
################################
