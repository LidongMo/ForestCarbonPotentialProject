...
presentForestCover = compositeImageNew.select(['HansenEtAl_TreeCover_Year2010']).divide(100).unmask().rename("treecover2010") # uniform with potential in the  0-1 scale
# Composite image to sample
compositeToUse = ee.Image("users/leonidmoore/ForestBiomass/20200915_Forest_Biomass_Predictors_Image").addBands(presentForestCover)
# FeatureCollection to sample
points = ee.FeatureCollection("users/leonidmoore/ForestBiomass/PlotsShapefiles/GFBI_Full_MAD25_AllYear_Mean_Aggre")
...
def extract_and_write_grid(n, grids, points, region):
    ...
    if len(results) > 0:
        ...
        df.to_csv("CovariatesTable/CovariatesExtractedTables/sampled_%d.csv" % n)
