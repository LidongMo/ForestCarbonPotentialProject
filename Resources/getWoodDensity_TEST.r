...
#The modification is replaced the wdData sources and also the calculation algorithms in side the function3
# however, it still needs improvement to make it applicable
#2018.April,24


getWoodDensity_LM <- function (genus, species, stand = NULL, family = NULL, region = "World",
addWoodDensityData = NULL)
{
    #modification 1#
    library(BIOMASS)
    setwd("/Users/LeonidMoore/Desktop/BIOMASS/")
    ...
    wdData_path   <- ("Resources/name_corrected_wooddensity_database_with_baad.csv")
    ...
    if (!is.null(addWoodDensityData)) {
        ...
        #write.csv(subWdData,"Resources/combined_corrected_wooddensity_database_with_baad.csv")
    }
    ...
    #give ID to each row of the species genus and species name#
    inputData$id             <- 1:length(species)
    #get the wood density data from family, genus and species level, wich matches the family,genus and species level#
    ...
    aggregatedNameFG         <- paste(selectData$family, selectData$genus, sep = "_")
    selectData               <- data.frame(selectData, aggregatedName,aggregatedNameFG)

    allMeanSp                <- unique(selectData[, c("family", "genus", "species","aggregatedName","aggregatedNameFG")])
    allMeanSp                <- allMeanSp[order(allMeanSp$aggregatedName), ]
    allMeanSp$meanWDsp       <- tapply(selectData$wd, selectData$aggregatedName,mean)
    allMeanSp$nIndSp         <- tapply(selectData$wd, selectData$aggregatedName,length)

    allMeanGen               <- unique(selectData[, c("family", "genus","aggregatedNameFG")])
    allMeanGen               <- allMeanGen[order(allMeanGen$genus), ]
    allMeanGen$meanWDgen     <- tapply(allMeanSp$meanWDsp, allMeanSp$aggregatedNameFG,mean)
    allMeanGen$nIndGen       <- tapply(allMeanSp$meanWDsp, allMeanSp$aggregatedNameFG,length)

    allMeanGen               <- allMeanGen[,c("family", "genus","meanWDgen","nIndGen")]
    allMeanSp                <- allMeanGen[,c("family", "genus","species","meanWDsp","nIndSp")]
