...
setwd("~/Github_Folder")
BiomassBiomeTPH = fread("BiomassCalculationData/GFBI_Biomass_Tree_Level_Biomass_New_PLT_Data_Frame_20181024.csv")[,-1]
...
woodDensityData = read.csv("BiomassCalculationData/original_wd_of_all_spsecies_GFBI.csv") 
# get the plot names
...
MultipleYearPlotFind            <- function(pn)
{
    ...
    perPlotDataFrame = fread(paste("PerPLT/PLT_",pn,".csv",sep=""))[,-1]
...
system.time(OutputList  <- mclapply(plotNames,MultipleYearPlotFind,mc.cores = 28, mc.preschedule=FALSE))
OutputDF = rbindlist(OutputList)
# write the data frame into local folder
write.csv(OutputDF,"BiomassCalculationData/GFBI_Plots_with_All_Years_Time_Series_Records_20190412_Lidong.csv")
